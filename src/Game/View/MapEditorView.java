...
public class MapEditorView extends JFrame {
    ...
    private void initCountryListeners() {
        this.btnAddCountry.addActionListener((ActionEvent e) -> {
            ...
            if ((comboNeighbourCountry.getSelectedIndex() != -1)
                && !comboModelNeighbourCountries
                .getElementAt(comboNeighbourCountry.getSelectedIndex())
        ...
        this.btnUpdateCountry.addActionListener((ActionEvent e) -> this.updateCountry());

        this.listCountries.addListSelectionListener((ListSelectionEvent e) -> this.selectCountryFromUser());

        this.btnDeleteCountry.addActionListener((ActionEvent e) -> this.deleteCountry());

        this.listNeighbourCountries.addListSelectionListener((ListSelectionEvent e) ->
            this.selectTheNeighbouringCountry());
    }

    /** select the neighbouring country from the UI */
    private void selectTheNeighbouringCountry() {
        this.selectedNeighbouringCountry = listNeighbourCountries.getSelectedIndex();
    }

    /**
     * Delete the selected country from the list.
     * Don't delete if it is part of any sub-connected graph
     */
    @SuppressWarnings("unchecked")
     ...
     * Update all the UI element when a country is selected.
     */
    @SuppressWarnings("unchecked")
    private void selectCountryFromUser() {
    ...
    /** Add a continent from the UI values */
    @SuppressWarnings("unchecked")
    ...
    private void initContinentListeners() {
        this.btnAddContinent.addActionListener((ActionEvent e) -> this.addContinent());

        this.listContinents.addListSelectionListener((ListSelectionEvent e) -> this.selectContinent());

        this.btnDeleteContinent.addActionListener((ActionEvent e) -> this.deleteContinent());

        this.btnUpdateContinent.addActionListener((ActionEvent e) -> this.updateContinent());
    }
